...
def dict_occurent_users():
    occurent_users = {}
    with open("/dlabdata1/youtube_large/jouven/channels_more_10k/duplicate_users.pkl",'rb') as f:
...
def filtered_channels_index_id_mapping():
    with open("/dlabdata1/youtube_large/jouven/channels_more_10k/channels_more_10k.pkl",'rb') as f:
        channels_id = pickle.load(f)
    f.close()
    channels_id = sorted(channels_id)
    # Dictionnary mapping the channel id to an integer corresponding to the row of the sparse matrix.
...
def video_id_to_channel_id():
    return pd.read_pickle("/dlabdata1/youtube_large/jouven/channels_more_10k/video_to_channel_mapping_filtered.pkl")
